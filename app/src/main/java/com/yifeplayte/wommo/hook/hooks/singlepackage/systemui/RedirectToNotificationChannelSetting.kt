...
object RedirectToNotificationChannelSetting : BaseHook() {
    ...
    override fun hook() {
        ...
        loadClass("com.android.systemui.statusbar.notification.row.MiuiNotificationMenuRow").methodFinder()
        ...
        loadClass("com.android.systemui.statusbar.notification.NotificationSettingsHelper").methodFinder()
            .filterByName("startAppNotificationSettings").first().createHook {
                before { param ->
                    ...
                    val userHandleCurrent = getStaticObjectOrNullAs<UserHandle>(UserHandle::class.java, "CURRENT")
                    invokeMethodBestMatch(appContext, "startActivityAsUser", null, intent, userHandleCurrent)
                    param.result = null
                }
            }
    }
