...
object ExposureRefreshForNonMIUIWidget : BaseMultiHook() {
        ...
        clazzAppWidgetItemInfo.methodFinder().filterByName("parseWidgetMetaData").first().createHook {
            after {
                val receiverInfo = appContext.packageManager.getReceiverInfo(
                    ObjectUtils.getObjectOrNull(it.thisObject, "provider") as ComponentName,
                ...
                if (!isRealMIUIWidget) {
                    ObjectUtils.setObjectUntilSuperclass(it.thisObject, "isMIUIWidget", true)
                    ObjectUtils.setObject(it.thisObject, "miuiWidgetRefresh", "exposure")
                    ObjectUtils.setObject(it.thisObject, "miuiWidgetRefreshMinInterval", 10000)
                }
            }
        }
        clazzAppWidgetItemInfo.methodFinder().filterByName("obtainMiuiWidgetUpdateIntent").first().createHook {
            after { param ->
                if (!(XposedHelpers.getAdditionalInstanceField(param.thisObject, "isRealMIUIWidget") as Boolean)) {
    ...
    private fun android() {
        ...
        loadClass("com.android.server.am.ActivityManagerService").methodFinder().filterByName("broadcastIntentLocked").toList()
            .createHooks {
                before {
        ...
        loadClass("com.android.server.appwidget.AppWidgetServiceImplStubImpl").methodFinder().filterByName("isForMiui").first()
            .createHook {
